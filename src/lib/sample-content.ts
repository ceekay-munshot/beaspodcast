...
// ─────────────────────────────────────────────────────────────────────────────
// Sample content fixtures — TEST-ONLY.
//
// This is the rich, hand-authored sample lineup (a full episode with transcript,
// analysed summaries with ideas/insight/quant, and a complete weekly edition)
// that the export/email/PDF renderers are exercised against. It is deliberately
// decoupled from the live roster in `mock-data.ts`: the app ships real shows and
// loads real episodes from the feeds, while these fixtures give the rendering
// tests a stable, content-rich input that never changes when the roster changes.
//
// Nothing in the app imports this module — only the render smoke tests do.
// ─────────────────────────────────────────────────────────────────────────────

export const SAMPLE_PODCASTS: Podcast[] = [
  {
    id: 'stratechery',
    title: 'Stratechery',
    author: 'Ben Thompson',
...
export const SAMPLE_EPISODES: Episode[] = [
  ...
  // ── 5. Acquired — analysed, summary only (no ideas pitched) ────────────────
  {
    id: 'ep-acquired-tsmc',
    podcastId: 'acquired',
...
export const SAMPLE_WEEKLY: WeeklySummary = {
  id: 'wk-2026-05-30',
  ...
  overview: [
    ...
    `On the investing side, Invest Like the Best circled the same idea from the other end of the size spectrum: durable returns come from owning the right thing for a very long time and staying liquid when others are forced to sell. Acquired's TSMC deep-dive supplied the concrete case study — process leadership that compounds, wrapped in geopolitical concentration risk.`,
  ...
  contradictions: [
    ...
    `Stratechery says owning the model is a weak position; other shows framed frontier-model ownership as the enterprise kingmaker. Worth holding both.`,
  ],
  mentions: {
    people: ['Sam Altman', 'Morris Chang', 'Brian Janous', 'Satya Nadella', "Patrick O'Shaughnessy"],
